...
Interfacial properties, such as interfacial free energy between the solid and the liquid phase ($\gamma_{sl}$), play a central role when dealing with phenomena of nucleation and growth. Despite its importance from both theoretical models and practical applications, an accurate calculation of $\gamma_{sl}$ is a rather complicated task even for the case of simple elements. Furthermore, on the experimental side, techniques aimed at measuring this quantity are complicated by the very strict control on all experimental parameters that must be achieved to obtain accurate data.
Several computational methods have been developed to calculate $\gamma_{sl}$, where a complete control on the experimental variables is possible. These methods includes \emph{capillary fluctuation method}, different sorts of so--called \emph{cleaving methods} and approaches based on \emph{classical nucleation theory}. All these schemes largely rely on molecular dynamics in conjunction with Monte Carlo simulations.

The capillary fluctuation method (briefly, CFM), introduced by~\textcite{HoytPRL2001:CFM} in 2001, employs an analysis of height fluctuations of the solid--liquid interface at equilibrium to extract the interface stiffness, $\sigma$.
