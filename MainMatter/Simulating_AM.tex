...
%\begin{itemize}
%    \item Most simulation approaches focus on the meso--scale (phase field simulation and/or finite elements methods).
%    \item Why we focus instead on atomistic techniques? Because we can get a better understanding of strongly out of equilibrium phenomena and also provide provide parameters for meso--scale simulations, such as
%    \begin{enumerate}
%        \item Diffusion coefficient
%        \item Viscosity
%        \item Solid-liquid interfaces properties: dynamical properties and thermodynamics, nucleation rate.
%    \end{enumerate}
%\end{itemize}
...
The CFM exploits the well--known result from capillary theory~\cite{Karma1993} for which the stiffness is directly related to the equilibrium fluctuation spectrum of a rough interface. In mathematical terms, for a quasi--two dimensional interface, this reads
\begin{equation}
